# Design Document: Measuring Harm Uplift on Dangerous Knowledge Queries

## 1. Purpose

This document is the method contract for **Measuring Harm Uplift on Dangerous Knowledge Queries**. It specifies what is
...
One-liner: Build a local red-team harness that measures uplift on dangerous-knowledge style queries with safety gates.

Hypothesis: Build a local red-team harness that measures uplift on dangerous-knowledge style queries with safety gates.
...
|---|---|---|
| Config schema | `harmup.configs.schema` | Typed dataclasses + role-keyed models |
| Loader | `harmup.configs.loader` | Hydra compose API |
| Device | `harmup.models.device` | MPS/CUDA/CPU resolution + fallback flag |
| Registry | `harmup.models.registry` | Pinned revisions + hardware fit |
| Hooks | `harmup.models.hooks` | Capture, steer, ablate, patch, attn intervene |
| Generation | `harmup.models.generation` | Chat template path recording |
| Data | `harmup.data` | Synthetic items, splits, manifests |
| Evaluation | `harmup.evaluation` | Metrics, failure harness, protocols |
| Ablation | `harmup.ablation` | Control / dropout / seed-sweep arms |
| Reporting | `harmup.reporting` | Aggregate → MD/TeX/figures |
| Cache | `harmup.cache` | Versioned artifact store |
...
`src/harmup/harmup/` are preserved across regeneration.
